...
{
  #matrix<-matrix[,c(1:500,ncol(matrix))]
  dd=dim(matrix)
...
select.forward.wrapper<- function(dattable)
{
  ...
  #subset <- forward.search(names(dattable)[1:1000], evaluator)
  return(subset)
}
...
ProcessData<-function(matrix,disc.method,attrs.nominal,flag=FALSE)
{
  nclass=ncol(matrix)
  matrix=data.frame(matrix)
  #---the class in the last column------------
  matrix[,nclass]=as.factor(matrix[,nclass])

  if(disc.method=="SD") # discretization mean+-sd
  {
    m3=matrix
    for(i in 1:(nclass-1))
    {
       if(!(i%in%attrs.nominal))
       {
         #tt=classIntervals(matrix[,i], n=5, style="sd")
         #tt=c(-Inf,tt$brks[c(3,7)],Inf)
         a1=mean(matrix[,i])
         a2=sd(matrix[,i])
         kk=1
         break1=c(-Inf,a1-kk*a2,a1+kk*a2,Inf)
         m3[,i] <- discretize(matrix[,i],method = "fixed", breaks=break1,include.lowest = TRUE)
       }
   }           
  }
  ...
  if(disc.method=="MDL")
  {
    m3 <- Discretize(as.formula(paste(names(matrix)[nclass],"~.")), data = matrix)
    #m3<-mdlp(matrix)$Disc.data
  }
...
#matrix1<-matrix[,c(1:500,ncol(matrix))]
#val <- cfs(as.formula(paste(names(matrix1)[ncol(matrix1)]," ~ .")), matrix1)
end_time <- Sys.time()
...
{
val <- reliefmy(matrix,nrow(matrix), 0,repet=1)
...
info.val<-as.data.frame(val$tabla)
return(info.val)
}
...
select.mrmr.filter<-function(matrix,disc.method,attrs.nominal,max.no.features)
{
  out=ProcessData(matrix,disc.method,attrs.nominal,FALSE)
  m3=out$m3
  sel.feature=out$sel.feature
  names(sel.feature)=names(m3)
  #algorithm
  dd=dim(m3)
  if(dd[2]>1)
  {
    out1=varrank(data.df = m3,
                 method = "peng",
                 variable.important = names(m3)[dd[2]],
                 discretization.method = "sturges",
                 algorithm = "forward", scheme = "mid",n.var =max.no.features)
    score=diag(out1$distance.m)
    num.feature=sel.feature[out1$ordered.var] #val - sorting
    info=data.frame(out1$ordered.var,score,num.feature)
  }
  else
  {
    info=data.frame(character(),numeric(),numeric())
