...
#           print(nearhit[i,])
    ...
    acum = as.matrix(acum)
    tabla = data.frame(names[-p], pesotota/repet,1:f,acum)
    colnames(tabla) = c("Biomarker","Weights","NumberFeature", "frequency")
    tabla = tabla[order(-tabla[, 2]), ]
    #cat("Features appearing in at least half of repetitions ordered by their average relevance weight: \n")
    print(tabla[tabla[, 4] >= repet/2, ])
    ...
    reltabla = tabla[relevant, 1]
    return (list(relevant=relevant,reltabla=reltabla,tabla=tabla[,-4]))
}
